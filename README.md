...
Optional live integrations (Gemini plan generation, Kroger pricing/barcode lookup) are wired in behind env vars — see [Optional integrations](#optional-integrations). They were implemented without access to real API keys in this environment, so the network calls themselves are unverified; the request/response parsing and fallback behavior are covered by unit tests.
...
- `VITE_GEMINI_API_KEY` — adds a "Generate my week with AI" button on the Setup screen. Gemini picks meal ids from the same diet-filtered pool the deterministic assembler uses (`src/integrations/gemini.ts`); the response is validated against real meal ids before being accepted, and any failure (bad key, malformed JSON, unknown id) silently falls back to the local assembler.
...
- `src/integrations/` — optional live API clients (Gemini, Kroger), each gated behind an env var and falling back to the offline domain layer on any error.
